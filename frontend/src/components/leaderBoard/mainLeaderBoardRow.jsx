  ...
  <AnimatePresence>
    {isOpen && (
      <>
        ...
        >
          <div className="w-11/12 max-w-4xl">{children}</div>
        </motion.div>
      </>
    )}
  </AnimatePresence>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        {[
          ...
          >
            ...
            <div
              className={`text-center font-semibold ${getRatingColor(
                user1[key]
              )}`}
            >
              {user1[key] || "N/A"}
...
const LeaderboardRow = ({ user, rank, userData }) => {
  ...
  const getProfileLink = (id, platform) => {
    ...
    switch (platform) {
      case "CodeChef":
        return `https://www.codechef.com/users/${id}`;
      case "LeetCode":
        return `https://leetcode.com/${id}`;
      case "CodeForces":
        return `https://codeforces.com/profile/${id}`;
      default:
        return "#";
    }
  };
      ...
      >
        ...
        <div className="flex items-center justify-evenly w-full sm:w-2/6 py-4 sm:py-0">
          {[
            {
              key: "codeChefRating",
              platformID: "codeChefID",
              label: "CodeChef",
            },
            {
              key: "leetCodeRating",
              platformID: "leetCodeID",
              label: "LeetCode",
            },
            {
