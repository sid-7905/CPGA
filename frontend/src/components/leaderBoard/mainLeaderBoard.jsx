    ...
    >
        ...
        <div className="flex items-center justify-center w-8">
            ...
            {/* <span className="text-gray-400 font-bold w-8 text-center">
                {(currentPage - 1) * usersPerPage + rank + 1}
            </span> */}
            ...
            <NavLink to={`/profile/${user._id}`}>
                <p className="text-white font-semibold hover:text-blue-500">{user.name}</p>
                <p className="text-gray-400 text-sm hover:text-blue-500">@{user.username}</p>
            </NavLink>
        </div>
        ...
        <div className="w-1/3 flex justify-evenly">
            {[{ key:'codeChefRating', platformID: 'codeChefID', label: 'CodeChef' },
                ...
                { key:'codeForcesRating', platformID: 'codeForcesID', label: 'CodeForces' }].map(platform => (
                <div key={platform.key} className="text-center">
                    <NavLink to={getProfileLink(user[platform.platformID], platform.label)}  target="_blank" className={`font-semibold ${user[platform.key] === 0 ? 'text-gray-500' : 'text-white hover:text-blue-500 ' }`}>
    ...
    filtered.sort((a, b) => {
      const sortValue =
        sortPlatform === 'totalRating'
          ? (a.codeChefRating + a.leetCodeRating + a.codeForcesRating) -
